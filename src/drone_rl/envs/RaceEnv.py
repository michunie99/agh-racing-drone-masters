...
class RaceAviary(BaseAviary):
    ...
    def _computeReward(self):
        ...
        if len(p.getContactPoints(bodyA=self.DRONE_IDS[0],
                                  physicsClientId=self.CLIENT)) != 0:
            collision_penaulty = min((dg/wg)**2, 20)
            reward -= collision_penaulty 
            self.infos["collision_penaulty"] = collision_penaulty 
            
        # Add reguralization for the angular speed
        omega = state[13:16]
        omega_norm = np.linalg.norm(omega)**2
        # print(omega_norm)
        omega_penaulty = omega_norm
        reward -= self.OMEGA_COEF * omega_penaulty
